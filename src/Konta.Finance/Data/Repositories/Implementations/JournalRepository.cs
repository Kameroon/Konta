...
public class JournalRepository : BaseRepository<JournalRepository>, IJournalRepository
{
    ...
    {
        const string sql = "SELECT * FROM Journals WHERE Id = @Id";
    ...
    {
        const string sql = "SELECT * FROM Journals WHERE TenantId = @TenantId AND Code = @Code";
    ...
    {
        const string sql = "SELECT * FROM Journals WHERE TenantId = @TenantId ORDER BY Code ASC";
    ...
    {
        const string sql = @"
            INSERT INTO Journals (Id, TenantId, Code, Name, Type, CreatedAt)
